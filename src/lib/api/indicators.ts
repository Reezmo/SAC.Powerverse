...
let MOCK_THANDI_TASKS: AppIndicator[] = Array.from({ length: 35 }, (_, i) => {
  const taskNames = [
    "Host Regional Cultural Workshops",
    "Distribute Local Arts Grants",
...
export async function getEntityIndicators(
  entityId: string,
  page = 1,
  pageSize = 10,
): Promise<{ data: AppIndicator[]; total: number }> {
  if (USE_MOCK_DATA) {
  ...
  return apiRequest(
    `/api/entities/${entityId}/indicators?page=${page}&pageSize=${pageSize}`,
    session?.token,
  );
}

export async function submitQuarterProof(
  indicatorId: string,
  quarter: number,
  formData: FormData,
): Promise<AppIndicatorQuarter> {
  if (USE_MOCK_DATA) {
  ...
  const session = await readSession();
  return apiRequest(
    `/api/indicators/${indicatorId}/quarters/${quarter}/proof`,
    session?.token,
    {
      method: "POST",
      body: formData,
...
export async function getIndicatorDetails(
  id: string,
): Promise<{ indicator: AppIndicator; quarters: AppIndicatorQuarter[] }> {
  if (USE_MOCK_DATA) {
...
export async function getIndicatorSummary(): Promise<IndicatorSummary[]> {
  if (USE_MOCK_DATA) {
    return [
      ...
      {
        entityId: "2",
        entityName: "National Heritage Council",
        percentComplete: 20,
        percentRemaining: 80,
        totalIndicators: 5,
      },
  ...
  const session = await readSession();
  return apiRequest<IndicatorSummary[]>(
    "/api/dsac/indicator-summary",
    session?.token,
  );
}
